...
def get_first_fish():
    ...
    fish = Animal.query.filter_by(animal_species="fish").first()
...
def get_young_animals():
    ...
    """
    animals = Animal.query.filter(Animal.birth_year > 2015).all()

    return animals

def get_j_names():
    """Return the humans with first names that start with 'J'."""
    js = Human.query.filter(Human.fname.startswith("J")).all()
...
def get_null_bdays():
    ...
    animals = Animal.query.filter(Animal.birth_year == None).all()
...
def get_fish_or_rabbits():
    ...
    rabbit_fish = Animal.query.filter( (Animal.animal_species == 'fish') | (Animal.animal_species == "rabbit") ).all()
...
def print_directory():
    ...
    for fname, lname, name, animal_species in directory:
        print(f"{directory.fname}{directory.lname} /n -{directory.name} '({directory.animal_species})'")
...
def find_humans_by_animal_species(species):
    ...
    for animal in animals:
        return  animal.human_id

        # ok i cant give you a "list of humans" but i can give you their
        # human_ids! lol 
